...
class GiteaProvider:
    ...
    def __init__(
        self, base_url: str, token: str = "", timeout: float = 30.0, personal: bool = False
    ) -> None:
        ...
        # иначе .../boris/api/v1/… даёт 404. Отбрасываем путь.
        parts = urlsplit(base_url.rstrip("/"))
        self.api_root = f"{parts.scheme}://{parts.netloc}"
        self.base_url = self.api_root
        # Личный источник: перечисляем репозитории владельца токена (включая
        # приватные) — а не ищем по всему инстансу, как для общей зоны.
    ...
    async def list_repositories(self) -> list[RepoRef]:
        # Личный источник — только свои репозитории владельца токена (включая
        # приватные), а не всё, к чему токен имеет доступ (организации, чужие
        # репозитории-коллаборации). Общий скан инстанса — поиск по открытым.
        if self.personal:
            endpoint = await self._own_repos_endpoint()
        else:
            endpoint = "/repos/search"
        repos: list[RepoRef] = []
        page = 1
        while True:
            response = await self._client.get(endpoint, params={"limit": _PAGE_SIZE, "page": page})
            response.raise_for_status()
            body = response.json()
            # /repos/search заворачивает список в {"data": [...]}, а
            # /users/{login}/repos отдаёт массив напрямую.
    ...
    async def _own_repos_endpoint(self) -> str:
        """Чей токен — того и репозитории. Спрашиваем у Gitea логин владельца
        токена и перечисляем именно его репозитории: /users/{login}/repos с
        авторизацией отдаёт и приватные. Логин не узнали — берём всё, что видит
        токен (/user/repos), это шире, но лучше, чем ничего."""
        response = await self._client.get("/user")
        response.raise_for_status()
        login = response.json().get("login") or ""
        return f"/users/{login}/repos" if login else "/user/repos"
