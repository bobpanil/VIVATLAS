    ...
    with make_session() as s:
        ...
        for i, name in enumerate(["airbnb", "starbucks", "scanner"], 1):
            ...
            art = Artifact(
                ...
                summary_normal=f"{name} описание",
            )
            s.add(art)
            s.flush()
            s.add(
...
class FakeEmbed:
    ...
    def __init__(self, similarity: float = 0.9) -> None:
        # Вектор карточек — [1,0]. Подбираем запрос так, чтобы косинус вышел
        # нужным: [cos, sin].
        import math
...
def good_payload(best_id: int) -> dict:
    return {
        "best": {"id": best_id, "why": "подходит", "limitations": "нет"},
        "alternatives": [],
        "rejected": [],
        "chain": [],
        "confidence": 0.9,
...
# --- порог решает, а не модель ---


async def test_low_similarity_means_no_match_without_asking_the_model(session):
    # Модель даже не спрашиваем: если бы спросили, она бы что-нибудь подобрала.
    text = FakeText(good_payload(1))
    r = await recommend(session, "конвертировать видео", FakeEmbed(0.3), text)

    assert r.no_match is True
    assert r.best is None
    assert text.calls == 0, "модель не должна вызываться, если порог не пройден"
    assert r.suggestions


async def test_high_similarity_asks_the_model(session):
    text = FakeText(good_payload(1))
    r = await recommend(session, "дизайн Airbnb", FakeEmbed(0.9), text)
...
async def test_invented_id_is_dropped(session):
    # Модель сослалась на инструмент, которого ей не показывали.
    text = FakeText(
        {
            "best": {"id": 999, "why": "прекрасный инструмент", "limitations": "нет"},
            "alternatives": [],
            "rejected": [],
            "chain": [],
            "confidence": 0.95,
    ...
    r = await recommend(session, "дизайн", FakeEmbed(0.9), text)

    assert r.dropped_ids == 1
    assert r.best is None
    assert r.no_match is True, "выдуманный инструмент не должен становиться ответом"
...
async def test_invented_alternative_is_dropped_but_best_survives(session):
    ...
    text = FakeText(
        {
            "best": {"id": real_id, "why": "ок", "limitations": "нет"},
            "alternatives": [{"id": 777, "why": "выдумка", "limitations": "нет"}],
            "rejected": [],
            "chain": [],
            "confidence": 0.9,
            "basis": "documentation",
...
async def test_best_is_not_duplicated_in_alternatives(session):
    ...
    text = FakeText(
        {
            "best": {"id": real_id, "why": "ок", "limitations": "нет"},
            "alternatives": [{"id": real_id, "why": "он же", "limitations": "нет"}],
            "rejected": [],
            "chain": [],
            "confidence": 0.9,
            "basis": "documentation",
...
async def test_confidence_is_clamped(session):
    ...
    r = await recommend(session, "дизайн", FakeEmbed(0.9), FakeText(payload))
...
async def test_empty_catalog_is_no_match(session):
    ...
    r = await recommend(session, "что угодно", FakeEmbed(0.9), FakeText(good_payload(1)))
    assert r.no_match is True
